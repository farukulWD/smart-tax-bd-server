 ...
 *    still gets its upload slots for a tax type nobody has curated in the
 *    admin yet.
...
// Keyed by the tax-type values that existed before `value` became free text.
export const TAX_TYPE_DOCUMENT_MAP: Partial<Record<string, string[]>> = {
...
/**
 * Documents added by order-level booleans rather than by a tax type. Seeded into the catalog so admins can rename/translate them, but the
